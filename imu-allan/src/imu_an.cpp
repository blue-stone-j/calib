...
imu::AllanAcc *acc_z;
double start_t;
bool start       = true;    // true: haven't received first imu msg; false: have received first imu msg
bool end         = false;   // false: received data isn't enough; true: have enough data
int max_time_min = 10;      // sample time, unit is minute
std::string data_save_path; // path to save data and result
...
void writeData1(const std::string sensor_name,        //
                const std::vector<double> &gyro_ts_x, // time
                const std::vector<double> &gyro_d)    // deviation
{
  std::ofstream out_t;
  std::ofstream out_x;
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber sub_imu = n.subscribe(IMU_TOPIC, 20000000, imu_callback, ros::TransportHints().tcpNoDelay());
  //    ros::Publisher pub = n.advertise< geometry_msgs::Vector3Stamped >(
  //    ALLAN_TOPIC, 2000 );
  ...
  ///
  gyr_x->calc();
  std::vector<double> gyro_v_x  = gyr_x->getVariance();  // sigma2, unit: (degree/hour)^2
  std::vector<double> gyro_d_x  = gyr_x->getDeviation(); // sigma, unit is degree/hour
  std::vector<double> gyro_ts_x = gyr_x->getTimes();     // period(virtual), unit is second
  ...
  acc_x->calc();
  std::vector<double> acc_v_x  = acc_x->getVariance();  // unit is m^2/s^4=(m/s^2)^2
  std::vector<double> acc_d_x  = acc_x->getDeviation(); // unit is m/s^2
  std::vector<double> acc_ts_x = acc_x->getTimes();     // period(virtual), unit is second
