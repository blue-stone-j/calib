...
class ImuProcess
{
 public:
  ...
  /* This value should be measured while the IMU remains stationary and it should euqal to gravity. If not, it is
     caused by scale error and it will be compensated later.
  */
                                                 ...
                                                 PointCloudXYZI &pcl_out)
{
  ...
  F_x.setIdentity();
  cov_w.setZero();
  /** In constant velocity model, bias_g represents angular velocity **/
  /** In constant velocity model，bias_a represents linear acceleration **/
...
void ImuProcess::propagation_and_undist(const MeasureGroup &meas, StatesGroup &state_inout, PointCloudXYZI &pcl_out)
{
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++)
  {
    ...
    /* in IMU model, bias_g represents the bias of gyroscope. */
    angvel_avr -= state_inout.bias_g;
    acc_avr = acc_avr / IMU_mean_acc_norm * G_m_s2 - state_inout.bias_a;

    // unit of acc is m/s^2 from now
  ...
  } // endfor: middle integration completed
  ...
  if (lidar_type != L515)
  {
    ...
    for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--)
    {
      ...
      } // endfor: have undistorted points between these two imu
    } // endfor: have undistorted all points
  }
}
